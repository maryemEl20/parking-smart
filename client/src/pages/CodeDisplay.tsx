...
useEffect(() => {
  const spotRef = ref(database, `ParkingLotDetails/Place${spotId}`);
  const unsubscribe = onValue(spotRef, (snapshot) => {
    const data = snapshot.val();
    if (!data) return;

    setAccessCode(data.accessCode || "12345");

    const startTime = new Date(data.startDateTime).getTime();
    const timer = setInterval(() => {
      const diffSeconds = Math.max(0, Math.floor((startTime - Date.now()) / 1000));
